...
class SyncApiData extends BaseCommand
{
    ...
    {
        return [
            'date-from' => 'required|date_format:Y-m-d|before_or_equal:date-to',
            'date-to'   => 'required|date_format:Y-m-d|after_or_equal:date-from',
        ];
    }
    ...
    {
        return [
            'date-from.required' => 'The --date-from parameter is required.',
            ...
            'date-to.required' => 'The --date-to parameter is required.',
    ...
    {
        $dateFrom = Carbon::parse($validatedArguments['date-from']);
        $dateTo   = Carbon::parse($validatedArguments['date-to']);
        $today    = Carbon::today();

        // Доп. проверка: даты не в будущем
